...
@DontTest void testThatWontRun() {
    1.shouldEqual(2); //doesn't matter, won't run anyway
}

@DontTest class TestThatWontRun : TestCase {
    override void test() {
...
@ShouldFail void testShouldFailWithAssertionInTestFunction() {
    assert(false);
}
...
@Types!(int, byte) void testTemplate(T)() {
    T.init.shouldEqual(0);
}

@Types!(A) void testTemplateWithTypeFromAnotherModule(T)() {
