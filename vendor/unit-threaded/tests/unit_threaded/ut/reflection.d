...
unittest {
    ...
    const actual = moduleTestClasses!(unit_threaded.ut.modules.module_with_tests).map!(
            a => a.name).array;
...
unittest {
    ...
    const actual = moduleTestFunctions!(unit_threaded.ut.modules.module_with_tests).map!(
            a => a.getPath).array;
    assertEqual(actual, expected);
}
...
unittest {
    ...
    const actual = moduleUnitTests!(unit_threaded.ut.modules.module_with_tests).map!(
            a => a.name).array;
    assertEqual(actual, expected);
}
...
version (unittest) {
    ...
    private void assertFail(TestCase test, string file = __FILE__, size_t line = __LINE__) {
        ...
        test.silence;
        assert(test() != [],
                file ~ ":" ~ line.to!string ~ " Expected test case "
                ~ test.getPath ~ " to fail but it didn't");
    }
...
unittest {
    ...
    const testData = allTestData!(unit_threaded.ut.modules.parametrized).filter!(
            a => a.name.endsWith("testValues")).array;
...
unittest {
    ...
    const testData = allTestData!(unit_threaded.ut.modules.parametrized).filter!(
            a => a.name.endsWith("testTypes")).array;
...
unittest {
    ...
    const testData = allTestData!(unit_threaded.ut.modules.parametrized).filter!(
            a => a.name.canFind("builtinIntValues")).array;
...
@("Tests can be selected by tags") unittest {
    ...
    assertEqual(createTestCases(testData,
            ["unit_threaded.ut.modules.tags.testMake", "@ninja"]).length, 0);
}
...
unittest {
    ...
    const testData = allTestData!(unit_threaded.ut.modules.parametrized).filter!(
            a => a.name.canFind("testValues")).array;
...
unittest {
    ...
    const testData = allTestData!(unit_threaded.ut.modules.parametrized).filter!(
            a => a.name.canFind("testTypes")).array;
...
@("Cartesian parameterized built-in values") unittest {
    ...
    const testData = allTestData!(unit_threaded.ut.modules.parametrized).filter!(
            a => a.name.canFind("cartesianBuiltinNoAutoTags")).array;

    auto tests = createTestCases(testData);
    tests.map!(a => a.getPath).array.shouldBeSameSetAs(addModPrefix(["foo.red",
            "foo.blue", "foo.green", "bar.red", "bar.blue", "bar.green"].map!(a => "cartesianBuiltinNoAutoTags." ~ a)
            .array, "unit_threaded.ut.modules.parametrized"));
    ...
    assertEqual(allTestData!(unit_threaded.ut.modules.parametrized)
            .filter!(a => a.name.canFind("cartesianBuiltinAutoTags")).array.find!(
                a => a.getPath.canFind("bar.green")).front.tags, ["bar", "green"]);
}
...
@("Cartesian parameterized function values") unittest {
    ...
    const testData = allTestData!(unit_threaded.ut.modules.parametrized).filter!(
            a => a.name.canFind("CartesianFunction")).array;

    auto tests = createTestCases(testData);
    tests.map!(a => a.getPath).array.shouldBeSameSetAs(addModPrefix(["1.foo",
            "1.bar", "2.foo", "2.bar", "3.foo", "3.bar"].map!(a => "testCartesianFunction." ~ a)
            .array, "unit_threaded.ut.modules.parametrized"));

    foreach (test; tests) {
        test.getPath.canFind("2.bar") ? assertPass(test) : assertFail(test);
    }

    assertEqual(testData.find!(a => a.getPath.canFind("2.bar")).front.tags, ["2", "bar"]);
...
@("issue 43") unittest {
    ...
    auto inStructTest = testData.find!(a => a.getPath.canFind("InStruct")).array.createTestCases[0];
...
@("@ShouldFail") unittest {
    ...
    auto willFail = testData.filter!(a => a.getPath.canFind("will fail")).array.createTestCases[0];
    assertPass(willFail);
}
...
@("@ShouldFailWith") unittest {
    ...
    auto doesntFail = testData.filter!(a => a.getPath.canFind(
            "ShouldFailWith that fails due to not failing")).array.createTestCases[0];
    assertFail(doesntFail);

    auto wrongType = testData.find!(a => a.getPath.canFind(
            "ShouldFailWith that fails due to wrong type")).array.createTestCases[0];
    assertFail(wrongType);

    auto passes = testData.find!(a => a.getPath.canFind("ShouldFailWith that passes"))
        .array.createTestCases[0];
    assertPass(passes);
}
...
@("@Flaky") unittest {
    ...
    auto flakyPasses = testData.filter!(a => a.getPath.canFind(
            "flaky that passes eventually")).array.createTestCases[0];
    assertPass(flakyPasses);

    auto flakyFails = testData.filter!(a => a.getPath.canFind(
            "flaky that fails due to not enough retries")).array.createTestCases[0];
