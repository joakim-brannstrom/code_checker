...
mixin template GenNumeric(T, T low, T high) {
    ...
    static assert(is(typeof(() { T[] res = frontLoaded(); })),
    ...
    T gen(ref Random gen) {
        ...
        this.value = _index < frontLoaded.length ? frontLoaded[_index++]
...
struct Gen(T, T low = 0, T high = 6.022E23)
        if (from!"std.traits".isFloatingPoint!T) {
    private static T[] frontLoaded() @safe pure nothrow {
        import std.algorithm : filter;
        import std.array : array;
...
struct Gen(T, size_t low = 0, size_t high = 32)
    ...
    static this() {
        ...
        charset = chain( // \t and \n
                iota(0x09, 0x0B), // \r
                iota(0x0D, 0x0E), // ' ' through '~'; next is DEL
                iota(0x20, 0x7F), // Vulgar fractions, punctuation, letters with accents, Greek
                iota(0xA1,
                    0x377), // More Greek
                iota(0x37A, 0x37F), iota(0x384, 0x38A), iota(0x38C,
                    0x38C), iota(0x38E, 0x3A1), // Greek, Cyrillic, a bit of Armenian
                iota(0x3A3, 0x52F), // Armenian
                iota(0x531,
                    0x556), iota(0x559, 0x55F), // Arabic
                iota(0xFBD3, 0xFD3F),
                iota(0xFD50, 0xFD8F), iota(0xFD92, 0xFDC7), // Linear B, included because it's a high character set
                iota(0x1003C, 0x1003D),
                iota(0x1003F, 0x1004D), iota(0x10050, 0x1005D), iota(0x10080,
                    0x100FA), // Emoji
                iota(0x1F300, 0x1F6D4)).map!(a => cast(dchar) a).array;
...
struct GenASCIIString(size_t low = 1, size_t high = 32) {
    ...
    static this() {
        ...
        GenASCIIString!(low, high).charSet = to!string(chain(iota(0x21, 0x7E)
...
struct Gen(T, size_t low = 1, size_t high = 1024)
    ...
    T gen(ref Random rnd) {
        value = _index < frontLoaded.length ? frontLoaded[_index++] : genArray(rnd);
        return value;
    }
...
struct Gen(T, T low = minimum!T, T high = maximum!T)
        if (from!"std.traits".isSomeChar!T) {
    private static T[] frontLoaded() @safe pure nothrow {
        return [];
    }
