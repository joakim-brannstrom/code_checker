...
DubInfo getDubInfo(string jsonString) @trusted {
    ...
    return DubInfo(packages.map!(a => DubPackage(a.byKey("name").str,
            a.byKey("path").str, a.getOptional("mainSourceFile"),
            a.getOptional("targetFileName"), a.byKey("dflags").jsonValueToStrings, a.byKey("importPaths")
            .jsonValueToStrings, a.byKey("stringImportPaths").jsonValueToStrings, a.byKey("files")
            .jsonValueToFiles, a.getOptional("targetType"), a.getOptionalList("versions"),
            a.getOptionalList("dependencies"), a.getOptionalList("libs"),
            a.byOptionalKey("active", true), //true for backwards compatibility
            )).filter!(a => a.active).array);
}
...
private string[] jsonValueToFiles(from!"std.json".JSONValue files) @trusted {
    ...
    return files.array.filter!(a => ("type" in a && a.byKey("type")
            .str == "source") || ("role" in a && a.byKey("role").str == "source")
            || ("type" !in a && "role" !in a)).map!(a => a.byKey("path").str).array;
}
...
private bool boolean(from!"std.json".JSONValue json) @trusted {
    ...
    enforce!JSONException(json.type == JSON_TYPE.TRUE
            || json.type == JSON_TYPE.FALSE, "JSONValue is not a boolean");
...
DubInfo getDubInfo(in bool verbose) {
    ...
    scope (exit)
        wait(pipes.pid); // avoid zombies in all cases
    string stdoutStr;
    string stderrStr;
    enum chunkSize = 4096;
    pipes.stdout.byChunk(chunkSize).joiner.map!"cast(immutable char)a".copy(appender(&stdoutStr));
    pipes.stderr.byChunk(chunkSize).joiner.map!"cast(immutable char)a".copy(appender(&stderrStr));
...
void dubify(ref from!"unit_threaded.runtime".Options options) {
    ...
    options.includes = dubInfo.packages.map!(
            a => a.importPaths.map!(b => buildPath(a.path, b)).array).reduce!((a, b) => a ~ b)
        .array;
