...
class PropertyException : Exception {
    this(in string msg, string file = __FILE__, size_t line = __LINE__, Throwable next = null) @safe pure nothrow {
 ...
 */
void check(alias F, int numFuncCalls = 100)(in uint seed = from!"std.random".unpredictableSeed,
    ...
    foreach (i; 0 .. numFuncCalls) {
        ...
        try {
            ...
            throw new UnitTestException(text("Property threw. Seed: ", seed,
                    ". Input: ", input(No.shrink), ". Message: ", t.msg), file, line, t,);
        }

        if (!pass) {
            throw new UnitTestException(text("Property failed. Seed: ", seed,
                    ". Input: ", input), file, line);
        }
    }
}
 ...
 */
void checkCustom(alias Generator, alias Predicate)(int numFuncCalls = 100,
        in string file = __FILE__, in size_t line = __LINE__) @trusted {
    ...
    foreach (i; 0 .. numFuncCalls) {
        ...
        try {
            ...
            throw new UnitTestException(text("Property threw. Input: ", object,
                    ". Message: ", t.msg), file, line, t,);
        }
...
private auto shrinkOne(alias F, int index, T)(T values) {
    ...
    return shrink!((a) { values[index] = a; return F(values.expand); })(orig);
...
private T shrinkImpl(alias F, T)(in T value, T[] candidates, T[][] oldParams = [])
    ...
    if (oldParams.canFind(params))
        return value;
    oldParams ~= params;

    // if it suddenly starts passing we've found our boundary value
    if (value < T.max && F(value + 1))
        return value;
    if (value > T.min && F(value - 1))
        return value;
    ...
    if (value < T.max / 2)
        attempts ~= cast(T)(value * 2);
    if (value < T.max)
        attempts ~= cast(T)(value + 1);
    if (value > T.min)
        attempts ~= cast(T)(value - 1);
    ...
    if (!F(min))
        return shrinkImpl!F(min, candidates, oldParams);
    if (!F(max))
        return shrinkImpl!F(max, candidates, oldParams);
...
private T shrinkImpl(alias F, T)(T value, T[] candidates, T[][] oldParams = [])
    ...
    auto snd = value[$ / 2 .. $];
    if (!F(fst))
        return shrinkImpl!F(fst, candidates);
    if (!F(snd))
        return shrinkImpl!F(snd, candidates);
    ...
    if (!F(value[0 .. $ - 1]))
        return shrinkImpl!F(value[0 .. $ - 1], candidates);
    if (!F(value[1 .. $]))
        return shrinkImpl!F(value[1 .. $], candidates);
    return candidates[0];
}
