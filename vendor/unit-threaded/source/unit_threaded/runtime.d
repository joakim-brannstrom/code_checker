...
Options getGenUtOptions(string[] args) {
    ...
    Options options;
    auto getOptRes = getopt(args, "verbose|v", "Verbose mode.", &options.verbose,
            "file|f", "The filename to write. Will use a temporary if not set.",
            &options.fileName, "I", "Import paths as would be passed to the compiler",
            &options.includes, "version", "Show version.", &options.showVersion,);

    if (getOptRes.helpWanted) {
        defaultGetoptPrinter("Usage: gen_ut_main [options] [testDir1] [testDir2]...",
                getOptRes.options);
        options.help = true;
        return options;
    }
...
from!"std.file".DirEntry[] findModuleEntries(in Options options) {
    ...
    if (!options.files.empty && options.dirs == ["."]) {
        return dubFilesToAbsPaths(options.fileName, options.files).map!toDirEntry.array;
    }
    ...
    foreach (dir; options.dirs) {
        ...
        bool isHiddenDir(string p) {
            return p.startsWith(".");
        }

        bool anyHiddenDir(string p) {
            return p.splitter(dirSeparator).canFind!isHiddenDir;
        }

        modules ~= normalised.filter!(a => !anyHiddenDir(a)).map!toDirEntry.array;
    }
...
string removePackage(string name) {
    ...
    return name.endsWith(toRemove) ? name.replace(toRemove, "") : name;
}
...
string[] dubFilesToAbsPaths(in string fileName, in string[] files) {
    ...
    // dub list of files, don't bother reading the filesystem since
    // dub has done it already
    return files.filter!(a => a != fileName).map!(a => removePackage(a))
        .map!(a => buildNormalizedPath(a)).array;
}
...
string[] findModuleNames(in Options options) {
    ...
    string relativeToImportDirs(string path) {
        foreach (string importPath; options.includes) {
            ...
            if (!importPath.endsWith(dirSeparator))
                importPath ~= dirSeparator;
    ...
    return findModuleEntries(options).filter!(a => a.baseName != "reggaefile.d")
        .filter!(a => a.absolutePath != options.fileName.absolutePath).map!(a => relativeToImportDirs(a.name))
        .map!(a => replace(a.stripExtension, dirSeparator, ".")).array;
}
