...
struct TestSuite {
    ...
    bool run() {
        ...
        import unit_threaded.io : writelnRed, writeln, writeRed, write,
        ...
        if (_options.random)
            _output.writeln("Tests were run in random order. To repeat this run, use --seed ",
                    _options.seed, "\n");
    ...
    shared static this() {
        ...
        Runtime.moduleUnitTester = () {
            ...
            version (unitUnthreaded)
                ...
                const singleThreaded = Runtime.args.canFind("-s")
                    || Runtime.args.canFind("--single");
...
private bool moduleUnitTester() {
    ...
    foreach (module_; ModuleInfo) {
        if (module_ && module_.unitTest && module_.name.startsWith("unit_threaded")
                && // we want to run the "normal" unit tests
