...
void threadWriter(alias OUT, alias ERR)(from!"std.concurrency".Tid tid) {
    ...
    while (!done) {
        receive((string msg, Tid originTid) {
            ...
            if (currentTid == originTid)
                ...
                if (originTid !in outputs)
                    outputs[originTid] = typeof(outputs[originTid]).init;
                outputs[originTid].store(msg);
            }
        }, (ThreadWait w) { tid.send(ThreadStarted()); }, (ThreadFinish f) {
            done = true;
        }, (Flush f, Tid originTid) {

            if (originTid in outputs)
                outputs[originTid].flush;
