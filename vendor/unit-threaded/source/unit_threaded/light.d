 ...
 */
void check(alias F)(int numFuncCalls = 100, in string file = __FILE__, in size_t line = __LINE__) @trusted {
...
void shouldNotEqual(V, E)(in auto ref V value, in auto ref E expected,
...
void shouldBeIn(T, U)(in auto ref T value, U container, in string file = __FILE__,
        in size_t line = __LINE__) if (!isLikeAssociativeArray!(U, T)) {
...
void shouldThrowExactly(T : Throwable = Exception, E)(lazy E expr,
    ...
    const sameType = () @trusted{
        return throwable !is null && typeid(throwable) == typeid(T);
    }();
...
void shouldBeSameSetAs(V, E)(in auto ref V value, in auto ref E expected,
        in string file = __FILE__, in size_t line = __LINE__) {
    assert_(isSameSet(value, expected), file, line);
}

/// Assert that value is not the same set as expected.
void shouldNotBeSameSetAs(V, E)(in auto ref V value, in auto ref E expected,
        in string file = __FILE__, in size_t line = __LINE__) {
    assert_(!isSameSet(value, expected), file, line);
}
