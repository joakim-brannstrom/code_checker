...
const(TestData)[] allTestData(MOD_STRINGS...)()
    ...
    string getModulesString() {
        string[] modules;
        foreach (i, module_; MOD_STRINGS)
            modules ~= "module%d = %s".format(i, module_);
        return modules.join(", ");
    }

    enum modulesString = getModulesString;
    mixin("import " ~ modulesString ~ ";");
    mixin("return allTestData!(" ~ MOD_STRINGS.length.iota.map!(i => "module%d".format(i))
            .join(", ") ~ ");");
}
...
const(TestData)[] allTestData(MOD_SYMBOLS...)()
    ...
    return allTestsWithFunc!"moduleTestClasses"
        ~ allTestsWithFunc!"moduleTestFunctions" ~ allTestsWithFunc!"moduleUnitTests";
}
...
TestData[] moduleUnitTests(alias module_)() pure nothrow {
    ...
    void function() getUDAFunction(alias composite, alias uda)() pure nothrow {
        import std.traits : fullyQualifiedName, moduleName, isSomeFunction,
            hasUDA;
        ...
        foreach (index, eLtEstO; __traits(getUnitTests, composite)) {
            ...
            static if (!dontTest) {
                ...
                enum shouldFail = hasUDA!(eLtEstO, ShouldFail)
                        || hasUtUDA!(eLtEstO, ShouldFailWith);
                ...
                static if (valuesUDAs.length == 0) {
                    testData ~= TestData(name, () {
                        ...
                        eLtEstO();
                    }, hidden, shouldFail, singleThreaded, builtin, suffix,
                            tags, exceptionTypeInfo, flakyRetries);
                } else {
                    import std.range;
                    ...
                    foreach (comb; aliasSeqOf!prod) {
                        ...
                        testData ~= TestData(name ~ "." ~ valuesName, () {
                            ...
                            eLtEstO();
                                ...
                                realTags, exceptionTypeInfo, flakyRetries);
                    }
                }
            }
        }
    ...
    void addUnitTestsRecursively(alias composite)() pure nothrow {
        ...
        foreach (member; __traits(allMembers, composite)) {
            ...
            static if (notPrivate && // If visibility of the member is deprecated, the next line still returns true
                    // and yet spills deprecation warning. If deprecation is turned into error,
                    // all works as intended.
                    __traits(compiles, __traits(getMember, composite,
                        member)) && __traits(compiles, __traits(allMembers, __traits(getMember, composite, member)))
...
private template PassesTestPred(alias module_, alias pred, string moduleMember) {
    ...
    static if (!__traits(compiles, I!(__traits(getMember, module_, moduleMember)))) {
        ...
        template canCheckIfSomeFunction(T...) {
            enum canCheckIfSomeFunction = T.length == 1 && __traits(compiles,
                        isSomeFunction!(T[0]));
        }
            ...
            static if (!canCheckIfSomeFunction!member) {
                ...
                static if (isSomeFunction!member) {
                    ...
                    static if (isAggregateType!member)
                        ...
                        enum isPrivate = !__traits(compiles,
                                    __traits(getMember, module_, moduleMember));
                } else {
                    enum isPrivate = !__traits(compiles, __traits(getMember,
                                module_, moduleMember));
                }
            }
        }
...
TestData[] moduleTestFunctions(alias module_)() pure {
    ...
    template isTestFunction(alias module_, string moduleMember) {
        ...
        static if (.isPrivate!(module_, moduleMember)) {
            ...
            enum isTestFunction = hasTestPrefix!(module_, moduleMember) && types.length > 0;
        } else {
            enum isTestFunction = false;
        }
    ...
    template hasTestPrefix(alias module_, string memberName) {
        ...
        static if (memberName.length >= minSize
                && memberName[0 .. prefix.length] == prefix && isUpper(memberName[prefix.length])) {
...
private TestData[] createFuncTestData(alias module_, string moduleMember)() {
    ...
    */
    ...
    static if (isRegularFunction) {
        ...
        static if (arity == 0) // the reason we're creating a lambda to call the function is that test functions
            ...
            static if (!hasAttributesForAllParams) {
                ...
                pragma(msg, text("Warning: ", moduleMember,
                ...
                static if (arity == 1) {
                    ...
                    enum prod = [GetAttributes!(module_, moduleMember, params[0])].map!(
                                a => tuple(a));
                } else {
                    import std.conv : text;

                    mixin(`enum prod = cartesianProduct(` ~ params.length.iota.map!(
                            a => `[GetAttributes!(module_, moduleMember, params[` ~ guaranteedToString(a) ~ `])]`)
                            .join(", ") ~ `);`);
                }
                ...
                foreach (comb; aliasSeqOf!prod) {
                    ...
                    testData ~= memberTestData!(module_, moduleMember, extraTags)( // func(value0, value1, ...)
                            () { func(comb.expand); }, valuesName);
                }
...
private TestData memberTestData(alias module_, string moduleMember, string[] extraTags = [])(
    ...
    enum shouldFail = HasAttribute!(module_, moduleMember, ShouldFail)
            || hasUtUDA!(mixin(moduleMember), ShouldFailWith);
    enum flakyRetries = getFlakyRetries!(mixin(moduleMember));

    return TestData(fullyQualifiedName!module_ ~ "." ~ moduleMember, testFunction, HasAttribute!(module_, moduleMember,
            HiddenTest), shouldFail, singleThreaded, builtin, suffix,
            tags ~ extraTags, exceptionTypeInfo, flakyRetries);
}
...
private int getFlakyRetries(alias test)() {
    ...
    static assert(flakies.length == 0 || flakies.length == 1, text("Only 1 @Flaky allowed, found ",
            flakies.length, " on ", __traits(identifier, test)));
