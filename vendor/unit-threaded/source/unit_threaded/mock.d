...
string implMixinStr(T)() {
    ...
    import std.traits : functionAttributes, FunctionAttribute, Parameters,
        ReturnType, arity;
    ...
    string getOverload(in string memberName, in int i) {
        return `Identity!(__traits(getOverloads, T, "%s")[%s])`.format(memberName, i);
    }
            ...
            static if (__traits(isVirtualMethod, member)) {
                foreach (i, overload; __traits(getOverloads, T, memberName)) {
                    ...
                    static if (!(functionAttributes!member & FunctionAttribute.const_)
                        ...
                        lines ~= "private alias %s_parameters = Parameters!(%s);".format(overloadName,
                                overloadString);
                        lines ~= "private alias %s_returnType = ReturnType!(%s);".format(overloadName,
                                overloadString);
                        ...
                        lines ~= `override ` ~ overloadName ~ "_returnType " ~ memberName ~ typeAndArgsParens!(
                                Parameters!overload)(overloadName) ~ " "
                        ...
                        lines ~= tryIndent ~ `    calledValues ~= tuple` ~ argNamesParens(
                                arity!member) ~ `.to!string;`;
...
mixin template MockImplCommon() {
    ...
    void expectCalled(string func, string file = __FILE__, size_t line = __LINE__, V...)(
            auto ref V values) {
        expect!func(values);
        verify(file, line);
        _verified = false;
    ...
    void verify(string file = __FILE__, size_t line = __LINE__) @safe pure {
        ...
        for (int i = 0; i < expectedFuncs.length; ++i) {
            ...
            if (i >= calledFuncs.length)
                fail("Expected nth " ~ i.to!string ~ " call to " ~ expectedFuncs[i] ~ " did not happen",
                        file, line);

            if (expectedFuncs[i] != calledFuncs[i])
                fail("Expected nth " ~ i.to!string ~ " call to " ~ expectedFuncs[i]
                        ~ " but got " ~ calledFuncs[i] ~ " instead", file, line);
            ...
            if (expectedValues[i] != calledValues[i] && expectedValues[i] != "")
                throw new UnitTestException([expectedFuncs[i] ~ " was called with unexpected " ~ calledValues[i],
                        " ".repeat.take(expectedFuncs[i].length + 4)
                        .join ~ "instead of the expected " ~ expectedValues[i]], file, line);
        }
    }
}
...
struct ReturnValues(string function_, T...)
        if (from!"std.meta".allSatisfy!(isValue, T)) {
            ...
            auto opDispatch(string funcName, V...)(auto ref V values) {
                ...
                static if (T.length > 0) {
                    ...
                    if (_returnValues.length == 0)
                        return typeof(_returnValues[0]).init;
 ...
 */
auto mockStruct(T...)()
        if (T.length > 0 && from!"std.meta".allSatisfy!(isReturnValue, T)) {
...
@safe pure unittest {
    auto m = mockStruct!(ReturnValues!("length", 5), ReturnValues!("greet", "hello"));
    assert(m.length == 5);
