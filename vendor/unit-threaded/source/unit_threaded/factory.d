...
from!"unit_threaded.testcase".TestCase[] createTestCases(
        in from!"unit_threaded.reflection".TestData[] testData, in string[] testsToRun = []) {
    ...
    foreach (const data; testData) {
        ...
        if (test !is null)
            tests[test] = true; //can be null if abtract base class
    }
...
package from!"unit_threaded.testcase".TestCase createTestCase(
    ...
    TestCase createImpl() {
        import unit_threaded.testcase : BuiltinTestCase, FunctionTestCase,
            ShouldFailTestCase, FlakyTestCase;
        ...
        if (testData.isTestClass)
            ...
            testCase = testData.builtin ? new BuiltinTestCase(testData)
                : new FunctionTestCase(testData);

        version (unitThreadedLight) {
        } else
            assert(testCase !is null, text("Error creating test case with ",
                    testData.isTestClass ? "test class data: " : "data: ", testData));
...
private bool isWantedTest(in from!"unit_threaded.reflection".TestData testData,
    ...
    return isWantedNonTagTest(testData, normalToRun) && (tagsToRun.empty
            || tagsToRun.all!(t => matchesTags(t)));
}

private bool isWantedNonTagTest(
        in from!"unit_threaded.reflection".TestData testData, in string[] testsToRun) {

    import std.algorithm : any, startsWith, canFind;

    if (!testsToRun.length)
        return !testData.hidden; //all tests except the hidden ones
...
unittest {
    ...
    assert(isWantedTest(TestData("example.tests.pass.normal.unittest"), []));
    assert(!isWantedTest(TestData("tests.pass.attributes.testHidden", null, true  /*hidden*/ ),
            ["tests.pass"]));
    assert(!isWantedTest(TestData("", null, false  /*hidden*/ , false  /*shouldFail*/ ,
            false  /*singleThreaded*/ , false  /*builtin*/ , "" /*suffix*/ ), ["@foo"]));
    assert(isWantedTest(TestData("", null, false  /*hidden*/ , false  /*shouldFail*/ ,
            false  /*singleThreaded*/ , false  /*builtin*/ , "" /*suffix*/ , ["foo"]), ["@foo"]));
    ...
    assert(isWantedTest(TestData("", null, false  /*hidden*/ , false  /*shouldFail*/ ,
            false  /*singleThreaded*/ , false  /*builtin*/ , "" /*suffix*/ , ["bar"]), ["~@foo"]));

    // if hidden, don't run by default
    assert(!isWantedTest(TestData("", null, true  /*hidden*/ , false  /*shouldFail*/ ,
            false  /*singleThreaded*/ , false  /*builtin*/ , "" /*suffix*/ , ["bar"]), ["~@foo"]));
