...
class UnitTestException : Exception {
    ...
    this(in string[] msgLines, string file = __FILE__, size_t line = __LINE__, Throwable next = null) @safe pure nothrow {
 ...
 * Throws: UnitTestException on failure
 */
void shouldEqual(V, E)(auto ref V value, auto ref E expected, in string file = __FILE__,
...
void shouldNotEqual(V, E)(V value, E expected, in string file = __FILE__, in size_t line = __LINE__) {
    if (isEqual(value, expected)) {
        const msg = [
            ...
            "is not expected to be equal to:", formatValueInItsOwnLine("", expected).join("")
        ];
        throw new UnitTestException(msg, file, line);
    }
}
...
bool isEqual(V, E)(in auto ref V value, in auto ref E expected)
        if (!isObject!V && (!isInputRange!V || !isInputRange!E)
            && !isFloatingPoint!V && !isFloatingPoint!E && is(typeof(value == expected) == bool)) {
...
void shouldApproxEqual(V, E)(in V value, in E expected, string file = __FILE__,
...
bool isEqual(V, E)(V value, E expected) if (isObject!V && isObject!E) {
...
auto should(E)(lazy E expr) {
    ...
    struct Should {
        ...
        void throw_(T : Throwable = Exception)(in string file = __FILE__, in size_t line = __LINE__) {
            shouldThrow!T(expr, file, line);
        }

        void throwExactly(T : Throwable = Exception)(in string file = __FILE__,
                in size_t line = __LINE__) {
            shouldThrowExactly!T(expr, file, line);
        }

        void throwWithMessage(T : Throwable = Exception)(in string file = __FILE__,
                in size_t line = __LINE__) {
